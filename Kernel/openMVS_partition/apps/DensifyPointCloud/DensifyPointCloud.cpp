...
bool Initialize(size_t argc, LPCTSTR* argv)
{
	...
	config.add_options()
		...
		("pairpath", boost::program_options::value<std::string>(&OPT::strPairPathDirName), "pairpath directory")
		...
		("parts", boost::program_options::value<unsigned>(&OPT::nParts)->default_value(8),"numbers of partition.")
		;
...
int main(int argc, LPCTSTR* argv)
{
	...
	if ((ARCHIVE_TYPE)OPT::nArchiveType != ARCHIVE_MVS) {
		TD_TIMER_START();
		if (!scene.DenseReconstruction(OPT::nParts, OPT::strPairPathDirName))
			return EXIT_FAILURE;
		VERBOSE("Get imageSet partition");
